...
def generate_summary_stats(merged_df, eye_df, saccades_df, blinks_df, imu_df, output_dir, trial_name):
    # Saves a small CSV with average durations, diameters, velocities, etc.
    stats = {}
    try:
        if blinks_df is not None:
            stats['total_blinks'] = len(blinks_df)
        else:
...
def analyze_trial(trial_name, main_df, surface_df, eye_df, blinks_df, saccades_df, imu_df, gaze_df, trial_keys,
    ...
    try:
        ...
        return generate_summary_stats(merged_df, eye_df, saccades_df, blinks_df, imu_df, output_dir, trial_name)
...
def main():
    ...
    try:
        ...
        # Load surface gaze for normalized gaze outputs
        surf_gaze_path = os.path.join(args.surface_dir, "gaze.csv")
        all_surf_gaze_df = pd.read_csv(surf_gaze_path) if os.path.exists(surf_gaze_path) else None
        ...
        if anchor_row is not None:
            ...
            if lsl_file:
                ...
                if baseline_offset_ns is not None:
                    ...
                    if sub_surf is not None: sub_surf.to_csv(
                        os.path.join(trial_out_dir, f"{baseline_name}_normalized_fixation_slice.csv"), index=False)
                        
                    sub_surf_gaze = slice_baseline(all_surf_gaze_df)
                    if sub_surf_gaze is not None: sub_surf_gaze.to_csv(
                        os.path.join(trial_out_dir, f"{baseline_name}_normalized_gaze_slice.csv"), index=False)

                    sub_imu = slice_baseline(all_imu_df)
                    
                    sub_blinks = slice_baseline(all_blinks_df, 'start timestamp [ns]')
                    if sub_blinks is not None: sub_blinks.to_csv(
                        os.path.join(trial_out_dir, f"{baseline_name}_raw_blinks_slice.csv"), index=False)
    ...
    for row in start_events.itertuples(index=False):
        ...
        if sub_surf is not None: sub_surf.to_csv(os.path.join(trial_out_dir, f"{trial_name}_normalized_fixation_slice.csv"),
                                                 index=False)

        sub_surf_gaze = slice_df(all_surf_gaze_df)
        if sub_surf_gaze is not None: sub_surf_gaze.to_csv(os.path.join(trial_out_dir, f"{trial_name}_normalized_gaze_slice.csv"),
                                                 index=False)

        sub_imu = slice_df(all_imu_df)
        
        sub_blinks = slice_df(all_blinks_df, 'start timestamp [ns]')
        if sub_blinks is not None: sub_blinks.to_csv(os.path.join(trial_out_dir, f"{trial_name}_raw_blinks_slice.csv"),
                                                 index=False)
